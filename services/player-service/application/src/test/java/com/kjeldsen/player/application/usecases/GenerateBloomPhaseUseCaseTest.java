...
class GenerateBloomPhaseUseCaseTest {
    ...
    @Test
    @DisplayName("Generate inside GenerateBloomPhaseUseCase should throw if player not found.")
    void generate_should_throw_exception_if_player_not_found() {
        // Arrange
        int bloomYears = 1;
        int bloomSpeed = 1;
        int bloomStart = 1;
        Player.PlayerId playerId = new Player.PlayerId("1");
    ...
    void generate_should_call_generateAndStoreEventOfBloomPhase_and_save_to_repository() {
        ...
        when(mockedPlayerReadRepository.findOneById(playerId)).thenReturn(Optional.ofNullable(getPlayer(playerId)));
