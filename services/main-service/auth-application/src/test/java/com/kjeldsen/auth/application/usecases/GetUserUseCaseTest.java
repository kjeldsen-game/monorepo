...
class GetUserUseCaseTest {
    ...
    public void should_throw_error_when_user_is_not_logged_in() {
        when(mockedUserReadRepository.findByUserId("userId")).thenReturn(Optional.empty());
        assertEquals("User not logged in", assertThrows(RuntimeException.class, getUserUseCase::getCurrent).getMessage());
    }

    @Test
    @DisplayName("Should get a user when user is logged in")
    public void should_get_user_when_user_is_logged_in() {
        User user = new User();
        user.setEmail("email");
    ...
    public void should_throw_error_when_user_not_found_by_email() {
    ...
    public void should_return_user_when_user_found_by_email() {
        User user = new User();
        user.setEmail("email");
        user.setPassword("password");
        user.setId("userId");
        when(mockedUserReadRepository.findByEmail("email")).thenReturn(Optional.of(user));
        User res = getUserUseCase.getUserByEmail("email");
