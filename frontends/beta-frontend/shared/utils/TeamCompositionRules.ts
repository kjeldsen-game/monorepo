...
const checkTeamComposition = (teamPlayers: Player[]): CompositionError[] => {
  ...
  const totalSweepers = teamPlayers.filter(
    (player) => player.position === PlayerPosition.SWEEPER,
  ).length;
  if (totalSweepers > 1) {
  ...
  const totalLeftBacks = teamPlayers.filter(
    (player) => player.position === PlayerPosition.LEFT_BACK,
  ).length;
  const totalLeftWingBacks = teamPlayers.filter(
    (player) => player.position === PlayerPosition.LEFT_WINGBACK,
  ).length;
  ...
  const totalRightBacks = teamPlayers.filter(
    (player) => player.position === PlayerPosition.RIGHT_BACK,
  ).length;
  const totalRightWingBacks = teamPlayers.filter(
    (player) => player.position === PlayerPosition.RIGHT_WINGBACK,
  ).length;
  ...
  if (totalAllCenterMidfielders < 1) {
    errors.push({
      error: 'totalAllCenterMidfielders',
      value: totalAllCenterMidfielders,
    });
  }
  ...
  if (totalDefensiveOffensiveMidfielders > 1) {
    errors.push({
      error: 'totalDefensiveOffensiveMidfielders',
      value: totalDefensiveOffensiveMidfielders,
    });
  }

  // No more than 3 center midfielders
  const totalCenterMidfielders = teamPlayers.filter(
    (player) => player.position === PlayerPosition.CENTRE_MIDFIELDER,
  ).length;
  if (totalCenterMidfielders > 3) {
    errors.push({
      error: 'totalCenterMidfielders',
      value: totalCenterMidfielders,
    });
  }

  // Between 1 and 3 Center Backs
  const totalCenterBacks = teamPlayers.filter(
    (player) => player.position === PlayerPosition.CENTRE_BACK,
  ).length;
  ...
  const totalLeftMidfielders = teamPlayers.filter(
    (player) => player.position === PlayerPosition.LEFT_MIDFIELDER,
  ).length;
  const totalLeftWingers = teamPlayers.filter(
    (player) => player.position === PlayerPosition.LEFT_WINGER,
  ).length;
  ...
  const totalRightMidfielders = teamPlayers.filter(
    (player) => player.position === PlayerPosition.RIGHT_MIDFIELDER,
  ).length;
  const totalRightWingers = teamPlayers.filter(
    (player) => player.position === PlayerPosition.RIGHT_WINGER,
  ).length;
  ...
  const totalForwards = teamPlayers.filter(
  ...
  // Max 1 striker
  const totalStrikers = teamPlayers.filter(
    (player) => player.position === PlayerPosition.STRIKER,
  ).length;
  if (totalStrikers > 1) {
    errors.push({ error: 'totalStrikers', value: totalStrikers });
  }

  const totalGoalkeepers = teamPlayers.filter(
